...
sed -i '/uci commit system/i\uci set system.@system[0].hostname='Netgear_wndr4300_v2'' package/lean/default-settings/files/zzz-default-settings

# 版本号里显示一个自己的名字（281677160 build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i "s/OpenWrt /compiled by 281677160 on $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt By ixmu.net /g" package/lean/default-settings/files/zzz-default-settings
